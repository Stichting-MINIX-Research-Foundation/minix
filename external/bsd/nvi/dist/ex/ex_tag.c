 ...
 *
 * PUBLIC: int ex_tag_first __P((SCR *, const CHAR_T *));
 */
int
...
ex_tag_top(SCR *sp, EXCMD *cmdp)
{
	...
	tqp = TAILQ_LAST(&exp->tq, _tqh);
	tqp = TAILQ_PREV(tqp, _tqh, q);
	return tag_pop(sp, tqp, FL_ISSET(cmdp->iflags, E_C_FORCE));
}

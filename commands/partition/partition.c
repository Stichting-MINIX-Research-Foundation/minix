...
void write_table(void)
{
	...
	/* we have a abstract struct but it must conform to reality. each
	 * partition entry is 16 bytes and there are 4 of them.
	 */
	...
	printf("seeked to 0x%x, wrote 0x%x bytes, signature of 0x%x bytes\n",
		PART_TABLE_OFF, sizeof(newtable), sizeof(signature));
}

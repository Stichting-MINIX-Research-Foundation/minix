...
static ssize_t Write(void * buf, size_t count);
...
Write(void * buf, size_t count)
{
	...
	if(w < 0)
		err(1, "Write: write failed");
	if(w != count)
		errx(1, "Write: short write: %ld != %ld", w, count);

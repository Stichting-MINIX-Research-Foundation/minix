	...
	vir_bytes addr, socklen_t addr_len)
{
	...
	len = ((struct sockaddr *)buf)->sa_len;
	family = ((struct sockaddr *)buf)->sa_family;
	all = (verbose > 1);
	...
	case AF_LOCAL:
		...
		left = addr_len - (int)offsetof(struct sockaddr_un, sun_path);
		if (left > 0) {
			buf[addr_len] = 0; /* force null termination */
			put_buf(proc, "sun_path", PF_LOCADDR | PF_PATH,
			    (vir_bytes)((struct sockaddr_un *)buf)->sun_path,
	...
	case AF_INET:
		...
		if (addr_len == sizeof(struct sockaddr_in)) {
			put_value(proc, "sin_port", "%u",
			    ntohs(((struct sockaddr_in *)buf)->sin_port));
			put_in_addr(proc, "sin_addr",
			    ((struct sockaddr_in *)buf)->sin_addr);
		} else
			all = FALSE;
		break;
	case AF_INET6:

...
#define MAX_DIRECTORY_LENGTH 4096
	size_t pos = 0, cur_entry = 0, cpt;
	struct inode_dir_entry entries[MAX_DIRECTORY_LENGTH];
	int status = OK;
	...
	for (cur_entry = 0; status == OK; cur_entry++) {
		...
		if (entries[cur_entry].i_node->skip) {
			free(entries[cur_entry].r_name);
			goto read_next_inode;
		}
	}
	...
	size_t offset, int name_only)
{
	...
	if (dir_rec->data_length_l > 0) {
		i->i_node->extent = alloc_mem(sizeof(struct dir_extent));
	...
	struct dir_extent *extent, size_t *offset)
{
	...
	while (!done) {
		...
		if ((dir_rec->length_file_id == extent_rec->length_file_id) &&
			...
			cur_extent->next = alloc_mem(sizeof(struct dir_extent));

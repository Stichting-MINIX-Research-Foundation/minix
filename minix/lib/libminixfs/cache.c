 ...
 */
  ...
  unsigned int i, nr_before, nr_after, span, limit, nr_blocks;

  if (nblocks == 0)
	return;

  if (nblocks > LMFS_MAX_PREFETCH)
	nblocks = LMFS_MAX_PREFETCH;
  ...
  for (i = 1; i < nblocks; i++) {
	...
	if (block < base_block && block + LMFS_MAX_PREFETCH >= base_block)
		SET_BIT(blocks_before, base_block - block - 1);
	else if (block > base_block && block - LMFS_MAX_PREFETCH <= base_block)
		SET_BIT(blocks_after, block - base_block + 1);
  }

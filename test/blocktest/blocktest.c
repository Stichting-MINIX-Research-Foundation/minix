...
static void vir_limits(dev_t sub0_minor, dev_t sub1_minor, int part_secs)
{
	...
	subpart = part;
	subpart.base = ((u64_t)(subpart.base) + (sub_size));
	subpart.size = ((u64_t)(sub_size));
...
static void real_limits(dev_t sub0_minor, dev_t sub1_minor, int part_secs)
{
	...
	entry[0].lowsec = ((u64_t)(part.base) / (unsigned)(sector_size)) + 1;
	...
	if (res.type == RESULT_OK && (cmp64(subpart.base,
		((u64_t)(part.base) + ((1 + part_secs) * sector_size))) ||
		cmp64u(subpart.size, part_secs * sector_size))) {
	...
	u8_t *sec_ptr[2], int sectors, int pattern, u32_t ssum[5])
{
	...
	base_pos = ((u64_t)(base_pos) + (sector_size));
...
static void unaligned_pos1(void)
{
	...
	simple_xfer(driver_minor, ((u64_t)(base_pos) + (sector_size - min_read)),
	...
	simple_xfer(driver_minor, ((u64_t)(base_pos) + (min_read)),
	...
	simple_xfer(driver_minor, ((u64_t)(base_pos) + (min_read)), buf2_ptr,
		sector_size, FALSE, sector_size, &res);
...
static void unaligned_pos2(void)
{
	...
	if (may_write) {
		...
		simple_xfer(driver_minor, ((u64_t)(base_pos) + (max_size)),
	...
	simple_xfer(driver_minor, ((u64_t)(base_pos) + (max_size)), buf_ptr +
	...
	vir_xfer(driver_minor, ((u64_t)(base_pos) + (min_read)), iov, NR_IOREQS,
...
static void sweep_area(u64_t base_pos)
{
	...
	for (i = 0; i < 6; i++) {
		...
		simple_xfer(driver_minor, ((u64_t)(base_pos) + (sector_size * i)),
...
static void high_disk_pos(void)
{
	...
	base_pos = ((u64_t)(base_pos) - (((u64_t)(base_pos) % (unsigned)(sector_size))));

	/* The partition end must exceed 32 bits. */
	if (cmp64(((u64_t)(part.base) + (part.size)), base_pos) < 0) {
		test_group("high disk positions", FALSE);

		return;
	}

	base_pos = ((u64_t)(base_pos) - (sector_size * 8));
	...
	base_pos = ((u64_t)(base_pos) - (part.base));

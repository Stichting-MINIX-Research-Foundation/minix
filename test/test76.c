...
test76a(void)
{
	...
	FD_ZERO(&set);
	FD_SET(1, &set); /* reading from stdout should block */
	maxfd = 1;

	if (pipe(pfd) < 0) e(5);
	FD_SET(pfd[0], &set); /* reading from empty pipe should block */
	if (maxfd < pfd[0]) maxfd = pfd[0];

	if ((sfd = socket(AF_INET, SOCK_STREAM, 0)) < 0) e(6);

	memset(&sin, 0, sizeof(sin));
	sin.sin_family = AF_INET;
	/* Binding to an arbitrary port is fine. */
	if (bind(sfd, (struct sockaddr *)&sin, sizeof(sin)) < 0) e(7);

	if (listen(sfd, 1) < 0) e(8);
	...
	if (setitimer(ITIMER_REAL, &it, NULL) < 0) e(9);

	tv.tv_sec = 1;
	tv.tv_usec = 0;
	if ((r = select(maxfd + 1, &set, NULL, NULL, &tv)) >= 0) {
		printf("DEBUG: %d, %u, %u, %u\n", r, FD_ISSET(1, &set),
		    FD_ISSET(pfd[0], &set), FD_ISSET(sfd, &set));
		e(10);
	}
	if (errno != EINTR) e(11);
	...
	if (sigaction(SIGUSR1, &oact, NULL) < 0) e(15);
}
...
test76e(void)
{
	...
	it.it_interval.tv_sec = 0;
	it.it_interval.tv_usec = 1;
	if (setitimer(ITIMER_REAL, &it, NULL) < 0) e(3);
	...
	if (left > 0 && !FD_SET(1, &set)) e(6);

	if (sigaction(SIGUSR1, &oact, NULL) < 0) e(7);
}

...
for needed in sfdisk mcopy dd wget sha1sum
do
...
#
# Download the stage 1 bootloader 
#
if [ ! -f ${MLO} ]
then
	if ! wget -O $MLO http://www.minix3.org/arm/beagleboard-xm/MLO
	then
		echo "Failed to download MLO"
		rm -f $MLO
		exit 1
	fi
...
if [ ! -f ${UBOOT} ]
then
	if ! wget -O $UBOOT http://www.minix3.org/arm/beagleboard-xm/u-boot.img
	then
		echo "Failed to download $UBOOT"
		rm -f $UBOOT
		exit 1
	fi
...
#
# Now given the sizes avove use DD to create separate files representing
...
#
# Generate a script that can be used with sfdisk to partition the
# IMG and call sfdisk on that
cat > ${IMG_DIR}/sfdisk.input <<END_SFDISK
...
sfdisk --no-reread -q ${IMG} < ${IMG_DIR}/sfdisk.input
...
mcopy -bsp -i ${IMG_DIR}/fat.img  $MLO ::MLO
mcopy -bsp -i ${IMG_DIR}/fat.img $UBOOT ::u-boot.img

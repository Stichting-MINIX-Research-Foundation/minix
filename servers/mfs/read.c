 ...
 */
  ...
  if (excess < nr_indirects) {
	...
	bp = get_block_disk(rip->i_dev, b, iomode); /* get double indirect block */
  ...
  bp = get_block_disk(rip->i_dev, b, iomode);	/* get single indirect block */
  if(opportunistic && lmfs_dev(bp) == NO_DEV) {
	put_block(bp, INDIRECT_BLOCK);
	return NO_BLOCK;
  }
...
# define BLOCKS_MINIMUM		(nr_bufs < 50 ? 18 : 32)
  ...
  if(block_spec)
	  bp = get_block_disk(dev, block, PREFETCH);
  else
  ...
  if(block_spec)
	  return get_block_disk(dev, baseblock, NORMAL);

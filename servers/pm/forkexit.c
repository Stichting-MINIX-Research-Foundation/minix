 ...
 *   do_waitpid:	perform the WAITPID (or obsolete WAIT) system call
...
/* The process pointed to by 'mp' has forked.  Create a child process. */
  ...
  *rmc = *rmp;			/* copy parent's process slot to child's */
  ...
  m.VFS_PM_PROC = rmc->mp_endpoint;
  m.VFS_PM_PPROC = rmp->mp_endpoint;
 ...
 * Both WAIT and WAITPID are handled by this code. WAIT is obsolete.
 */
  register struct mproc *rp;
  int i, pidarg, options, children;

  /* Set internal variables, depending on whether this is WAIT or WAITPID. */
  /* XXX support for WAIT is obsolete as of 3.3.0 */
  pidarg  = (call_nr != PM_WAITPID ? -1 : m_in.PM_WAITPID_PID);	/* 1st param */
  options = (call_nr != PM_WAITPID ?  0 : m_in.PM_WAITPID_OPTIONS);/* 3rd param */

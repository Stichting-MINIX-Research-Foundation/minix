  ...
  if (rw_flag == READING) {
	if (vp->v_size == 0) {
		...
		if (find_filp(vp, W_BIT) != NULL) {
			...
			if (susp_count > 0)
				release(vp, OP_WRITE, susp_count);
		}
		return(r);
	}
	return(bytes);
...
int op;				/* OP_READ, OP_WRITE, or OP_OPEN */
int count;			/* max number of processes to release */
{
/* Check to see if any process is hanging on vnode 'vp'. If one is, and it
 * was trying to perform the operation indicated by 'op', release it.
 */

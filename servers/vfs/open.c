...
/* Perform the mkdir(name, mode) system call. */
  ...
  vname1_length = (size_t) job_m_in.m1_i1;
  dirmode = (mode_t) job_m_in.m1_i2;
...
/* XXX END OF OBSOLETE CODE AS OF 3.3.0 */
...
int do_lseek(void)
{
	return actual_lseek(job_m_in.VFS_LSEEK_FD, job_m_in.VFS_LSEEK_WHENCE,
		(off_t) job_m_in.VFS_LSEEK_OFF_LO);
}
...
int actual_llseek(struct fproc *rfp, int seekfd, int seekwhence, u64_t offset,
	u64_t *newposp)
{
/* Perform the llseek(2) system call. */
  register struct filp *rfilp;
  u64_t pos, newpos;
  int r = OK;
...
int do_llseek(void)
{
	...
	if ((r = actual_llseek(fp, job_m_in.VFS_LSEEK_FD,

...
void service_pm_postponed(void)
{
  ...
  case VFS_PM_EXEC:
	...
	r = pm_exec(exec_path, exec_path_len, stack_frame, stack_frame_len,
		&pc, &newsp, job_m_in.VFS_PM_EXECFLAGS);

	/* Reply status to PM */
	m_out.m_type = VFS_PM_EXEC_REPLY;
	m_out.VFS_PM_PROC = proc_e;
 ...
 */
  ...
  case VFS_PM_UNPAUSE:
	{
		endpoint_t proc_e = m_in.VFS_PM_PROC;

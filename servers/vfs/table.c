...
int (* const call_vec[NR_VFS_CALLS])(void) = {
	...
	CALL(55)		= do_fcntl,	/* XXX OBSOLETE as of 3.3.0 */
	CALL(VFS_COPYFD)	= do_copyfd,		/* copyfd(2) */
	CALL(VFS_PIPE2)		= do_pipe2,		/* pipe2(2) */
	CALL(VFS_UMASK)		= do_umask,		/* umask(2) */
	CALL(VFS_CHROOT)	= do_chroot,		/* chroot(2) */
	CALL(65)		= do_stat,	/* XXX OBSOLETE as of 3.3.0 */
	CALL(66)		= do_fstat,	/* XXX OBSOLETE as of 3.3.0 */
	CALL(67)		= do_lstat,	/* XXX OBSOLETE as of 3.3.0 */
	CALL(VFS_GETDENTS_330)	= do_getdents,		/* getdents(2) */
	CALL(VFS_FTRUNCATE_330)	= do_ftruncate,		/* ftruncate(2) */
	CALL(80)		= do_getdents,	/* XXX OBSOLETE as of 3.3.0 */
	CALL(81)		= do_llseek,	/* XXX OBSOLETE as of 3.3.0 */
	CALL(VFS_SELECT)	= do_select,		/* select(2) */
	CALL(VFS_FCHDIR)	= do_fchdir,		/* fchdir(2) */
	CALL(VFS_FSYNC)		= do_fsync,		/* fsync(2) */
	CALL(93)		= do_truncate,	/* XXX OBSOLETE as of 3.3.0 */
	CALL(94)		= do_ftruncate,	/* XXX OBSOLETE as of 3.3.0 */
	CALL(VFS_FCHMOD)	= do_chmod,		/* fchmod(2) */
	CALL(VFS_FCHOWN)	= do_chown,		/* fchown(2) */
	CALL(97)		= do_lseek,	/* XXX OBSOLETE as of 3.3.0 */
	CALL(VFS_UTIMENS)	= do_utimens,		/* [fl]utime[n]s(2) */
	CALL(VFS_FCNTL_330)		= do_fcntl,		/* fcntl(2) */
	CALL(VFS_TRUNCATE_330)	= do_truncate,		/* truncate(2) */

...
static char static_sparepages[ARM_PAGE_SIZE*STATIC_SPAREPAGES + ARM_PAGE_SIZE] __aligned(ARM_PAGE_SIZE);
...
static void vm_freepages(vir_bytes vir, vir_bytes phys, int pages, int reason)
{
	...
	if(pt_writemap(vmprocess, &vmprocess->vm_pt, vir,
		MAP_NONE, pages*ARM_PAGE_SIZE, 0, WMF_OVERWRITE) != OK)
	...
	for(i = 0; i < ARM_VM_DIR_ENTRIES; i++)
		if(pt->pt_pt[i])
			vm_freepages((vir_bytes) pt->pt_pt[i],
				pt->pt_dir[i] & ARM_VM_PDE_MASK, 1, VMP_PAGETABLE);

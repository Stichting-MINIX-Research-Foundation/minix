...
#if 0
			printf("VM: mapcache: cached page dev 0x%lx off 0x%llx not found\n", dev, dev_off);
#endif
...
#if 0
		if(hb->page->refcount != 2) {
			struct phys_region *ppr = hb->page->firstregion;
			printf("VM: unexpected: refcount %d; ", hb->page->refcount);
			printf("dev 0x%x offset 0x%llx caller %d ",
				dev, dev_off, caller->vm_endpoint);
			while(ppr) {
				printf("mapped in %d ",
					ppr->parent->parent->vm_endpoint);
				ppr = ppr->next_ph_list;
			}
			printf("\n");
		}
#endif
...
#if 0
	printf("setcache: 0x%llx@0x%x, mapped in at 0x%p, ino 0x%llx@0x%u\n",
		dev_off, dev, msg->m_u.m_vmmcp.block, ino_off, msg->m_u.m_vmmcp.ino);
#endif

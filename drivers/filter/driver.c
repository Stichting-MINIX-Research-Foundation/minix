...
static int driver_open(int which)
{
	...
	if(!size_known) {
		disk_size = part.size;
		size_known = 1;
		sectors = ((u64_t)(disk_size) / (unsigned)(SECTOR_SIZE));
		if(cmp64(((u64_t)(sectors) * (SECTOR_SIZE)), disk_size)) {
...
int read_write(u64_t pos, char *bufa, char *bufb, size_t *sizep, int request)
{
	...
	if (m1.BDEV_STATUS != (ssize_t) *sizep) {
		 ...
		 */
		if (m1.BDEV_STATUS > (ssize_t) *sizep ||
			cmp64(((u64_t)(pos) + (m1.BDEV_STATUS)), disk_size) < 0)
	...
	if (both) {
		...
		if (m2.BDEV_STATUS != (ssize_t) *sizep) {
			...
			/* As above */
			if (m2.BDEV_STATUS > (ssize_t) *sizep ||
					cmp64(((u64_t)(pos) + (m2.BDEV_STATUS)),
					disk_size) < 0)
				return bad_driver(DRIVER_BACKUP, BD_PROTO,
					EFAULT);

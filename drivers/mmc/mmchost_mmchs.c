...
#define SANE_TIMEOUT 5000000	/* 500 MS */
/*
...
/* Wait for a task completion interrupt. */
		...
		if (is_ipc_notify(ipc_status)) {
			...
			case HARDWARE:
				...
				if (v & mask) {
					...
					// mmc_log_debug(&log, "unexpected HW
					// interrupt 0x%08x mask 0X%08x\n", v,
					// mask);
					if (sys_irqenable(&hook_id) != OK)
						printf
	...
	while (1 == 1) {
		...
		if (v & mask) {
			...
			// mmc_log_debug(&log, "unexpected HW interrupt 0x%08x 
			// mask 0X%08x\n", v, mask);
		}
	}
	return 1;		/* unreached */
    ...
    uint32_t blknr, unsigned char *buf)
{
	...
	if (intr_wait(MMCHS_SD_IE_TC_ENABLE_CLEAR)) {
		...
		mmc_log_warn(&log, "Timeout waiting for transfer complete\n");
		return 1;
	}
	intr_assert(MMCHS_SD_IE_TC_ENABLE_CLEAR);
	set32(base_address + MMCHS_SD_IE, MMCHS_SD_IE_BWR_ENABLE,

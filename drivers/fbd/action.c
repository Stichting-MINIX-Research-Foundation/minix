	...
	u64_t *skip)
{
	...
	if (cmp64(pos, rule->start) > 0) {
		if (skip != NULL) *skip = ((u64_t)(pos) - (rule->start));
	...
	unsigned *UNUSED(count), size_t *UNUSED(size), u64_t *pos)
{
	 ...
	 */
	range = ((u64_t)(((u64_t)(((u64_t)(rule->params.misdir.end) - (rule->params.misdir.start))) + (1))) / (unsigned)(rule->params.misdir.align));
	...
	*pos = ((u64_t)(rule->params.misdir.start) + (((u64_t)(choice) * (rule->params.misdir.align))));
}
